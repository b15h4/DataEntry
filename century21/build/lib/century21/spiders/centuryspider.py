...
class CenturySpider(scrapy.Spider):
    ...
    def parse(self,response):
        ...
        Names = hxs.xpath("//li/a[@class='agent-name']/text()").extract()
        Phones = hxs.xpath("//li[@class='agent-phone-number']/text()").extract()
        Emails = hxs.xpath("//a[@class='agent-email']/@href").extract()
        Websites = hxs.xpath("//a[@class='agent-name']/@href").extract()        
        ...
        #Next-Page
        next_page = f'https://www.century21.ca/search/directory/Province-ON/page{str(self.pagenumber)}'
        if self.pagenumber <= 111:
            self.pagenumber += 1          
